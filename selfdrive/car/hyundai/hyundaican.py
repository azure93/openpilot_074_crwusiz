...
def create_lkas11(packer, car_fingerprint, bus, apply_steer, steer_req, cnt, enabled, lkas11, hud_alert,
  ...
  # ["CF_Lkas_FcwOpt_USM"] Description
  ...
  # ["CF_Lkas_SysWarning"] Description
    # SysWarning 4 = keep hands on wheel
    # SysWarning 5 = keep hands on wheel (red)
    # SysWarning 6 = keep hands on wheel (red) + beep
    # Note: the warning is hidden while the blinkers are on
  
  # ["CF_Lkas_Bca_R"] Description
    # This field is actually LdwsActivemode
    # Genesis and Optima fault when forwarding while engaged 
  
  if car_fingerprint in [CAR.SONATA20, CAR.SONATA20_HEV, CAR.PALISADE, CAR.SOUL_EV, CAR.SELTOS]:
  ...
  elif car_fingerprint in [CAR.K5, CAR.GRANDEUR_HEV]:
    values["CF_Lkas_Bca_R"] = 0
    values["CF_Lkas_FcwOpt_USM"] = 2
    values["CF_Lkas_LdwsOpt_USM"] = 2
  elif car_fingerprint == CAR.SANTAFE:
    values["CF_Lkas_FcwOpt_USM"] = 2 if enabled else 1
    values["CF_Lkas_LdwsOpt_USM"] = 2
  ...
  if car_fingerprint in CHECKSUM["crc8"]:
    # CRC Checksum as seen on 2019 Hyundai Santa Fe , soul , sonata20 , palisade
    dat = dat[:6] + dat[7:8]
