...
int main(int argc, char* argv[]) {
  ...
  const int MIN_VOLUME = LEON ? 15 : 12;
  ...
  while (!do_exit) {
    ...
    if (s->controls_timeout > 0) {
      ...
      if (s->controls_seen && s->vision_connected && strcmp(s->scene.alert_text2, "Controls Unresponsive") != 0) {
        ...
        snprintf(s->scene.alert_text1, sizeof(s->scene.alert_text1), "%s", "핸들을 즉시 잡아주세요");
        snprintf(s->scene.alert_text2, sizeof(s->scene.alert_text2), "%s", "컨트롤이 응답하지않음");
